    ...
    <section className={compact ? "gc-week-overview" : "gc-card p-4 sm:p-5"}>
      {compact ? null : (
        <div className="flex items-start justify-between gap-3">
          ...
          <Link href="/split/personal" className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-white/[0.08] bg-white/[0.04] text-emerald-200" aria-label="عدّل جدولك الشخصي"><PencilLine className="h-4 w-4" /></Link>
        </div>
      )}
        ...
        {orderedDays.map((day) => {
          ...
          return (
            <Link key={day.scheduleDate} href={`/split/personal?day=${weekday}`} className={`gc-week-day-card ${active ? "gc-week-day-card-active" : ""}`} aria-current={active ? "date" : undefined}>
              <span className={`block text-[10px] font-black uppercase tracking-wide ${active ? "text-emerald-200" : "text-neutral-500"}`}>{SHORT_DAY[weekday]}</span>
      ...
      {compact ? null : <Link href="/split/personal" className="mt-4 flex items-center gap-3 rounded-xl border border-white/[0.07] bg-white/[0.025] p-3.5 transition hover:border-emerald-300/25">
        <span className="grid h-9 w-9 place-items-center rounded-xl bg-emerald-300/10 text-emerald-200"><CalendarDays className="h-4 w-4" /></span>
